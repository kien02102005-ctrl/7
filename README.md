    ...
    <script>
            ...
            tasks.forEach(t => {
                ...
                if (t.files && t.files.length > 0) {
                    fileListHTML = t.files.map((f, index) => {
                        ...
                        return `
                        <div class="file-tag" style="gap: 4px;">
                            <i class="fa-solid fa-paperclip"></i> 
                            <a href="${f.data}" download="${f.name}" title="Tải file về máy">${f.name}</a>
                            <button class="btn-sm btn-view" onclick="viewFileOnline('${safeFileName}', '${f.data}')" title="Xem trực tiếp file"><i class="fa-solid fa-eye"></i> Xem</button>
                            ${!isAdmin ? `<i class="fa-solid fa-xmark" style="cursor:pointer; color:#ef4444; margin-left:2px;" onclick="removeTaskFile('${t.id}', ${index})" title="Xóa file"></i>` : ''}
                        </div>
                    `;
                    }).join('<br>');
                } else {
        ...
        function renderKanban() {
            ...
            tasks.forEach(t => {
                ...
                card.innerHTML = `
                    <div class="id">${t.id} ${overdue ? '<span class="badge-overdue" style="font-size:9px; padding:1px 4px;"><i class="fa-solid fa-triangle-exclamation"></i> Quá hạn</span>' : ''}</div>
                    <div class="title">${t.name}</div>
                    <div class="meta">
                        <span><i class="fa-regular fa-clock"></i> ${t.date}</span>
                        <span style="font-weight:bold; color:${t.priority === 'Cao' ? '#ef4444' : t.priority === 'Bình thường' ? '#f59e0b' : '#10b981'}">${t.priority}</span>
                    </div>
                `;
        ...
        function renderKPITable() {
            ...
            const targetNameDisp = document.getElementById('kpi-target-name-display');
            const typeBadgeDisp = document.getElementById('kpi-type-badge-display');
            
            targetNameDisp.innerText = kpiTargetUser;

            const targetUserInfo = registeredUsers.find(u => u.username === kpiTargetUser);
            const userKpiType = (targetUserInfo && targetUserInfo.kpiType) ? targetUserInfo.kpiType : 'staff';

            if (userKpiType === 'leader') {
                typeBadgeDisp.innerText = 'Lãnh đạo';
                typeBadgeDisp.className = 'kpi-badge-type type-leader';
            } else if (userKpiType === 'cleaner') {
                typeBadgeDisp.innerText = 'Lao Công';
                typeBadgeDisp.className = 'kpi-badge-type type-cleaner';
            } else {
                typeBadgeDisp.innerText = 'Cán bộ / Nhân viên';
                typeBadgeDisp.className = 'kpi-badge-type type-staff';
            }
            ...
            Object.keys(sectionsMap).sort().forEach(secKey => {
                ...
                let secHeaderHTML = `
                    <div class="kpi-section-title">
                        <span>MỤC ${secKey}: ${secTitleText} (${maxSecScore} ĐIỂM)</span>
                        ${isAdmin ? `
                            <div>
                                <button class="btn-sm btn-edit" onclick="editSectionInfo('${secKey}')"><i class="fa-solid fa-pen"></i> Sửa tiêu đề/điểm</button>
                                <button class="btn-sm btn-attach" onclick="addSubsection('${secKey}')"><i class="fa-solid fa-plus"></i> Thêm nhóm La Mã</button>
                            </div>
                        ` : ''}
                    </div>
                `;

                secContainer.innerHTML = secHeaderHTML;
                ...
                sectionsMap[secKey].forEach(sub => {
                    globalSubCount++;
                    tableHTML += `
                        <tr class="row-sub-header">
                            <td style="text-align: center;"><strong>${sub.code || globalSubCount}</strong></td>
                            <td colspan="2">
                            ...
                            ${isAdmin ? `
                                <td style="text-align: center;">
                                    <button class="btn-sm btn-edit" onclick="editSubsection('${sub.id}')"><i class="fa-solid fa-pen"></i></button>
                                    <button class="btn-sm btn-attach" onclick="addItemToSub('${sub.id}')"><i class="fa-solid fa-plus"></i></button>
                    ...
                    if (sub.items && sub.items.length > 0) {
                        sub.items.forEach((item, itemIdx) => {
                            const isUserEditable = (currentUser === kpiTargetUser);
                            ...
                            tableHTML += `
                                <tr>
                                    ...
                                    <td style="text-align: center; font-weight: 600;">${item.maxScore}</td>
                                    <td style="text-align: center;">
                ...
                secContainer.appendChild(tableContainer);
        ...
        function calculateKPITotals() {
            ...
            document.getElementById('kpi-total-max').innerText = totalMax.toFixed(1).replace('.0', '');
            document.getElementById('kpi-total-self').innerText = totalSelf.toFixed(1).replace('.0', '');
            document.getElementById('kpi-total-admin').innerText = totalAdmin.toFixed(1).replace('.0', '');

            updateKPIRankingDisplay(totalAdmin);
        }

        function updateKPIRankingDisplay(score) {
            document.getElementById('rank-check-1').innerHTML = '';
            document.getElementById('rank-check-2').innerHTML = '';
            document.getElementById('rank-check-3').innerHTML = '';
            document.getElementById('rank-check-4').innerHTML = '';

            const resultBadge = document.getElementById('kpi-ranking-result-display');
            
            const checkMark = '<i class="fa-solid fa-circle-check" style="color: #10b981; font-size: 18px;"></i>';

            if (score >= 90) {
                document.getElementById('rank-check-1').innerHTML = checkMark;
                resultBadge.innerText = 'Hoàn thành xuất sắc nhiệm vụ';
                resultBadge.className = 'ranking-result-badge rank-excel';
            } else if (score >= 75) {
                document.getElementById('rank-check-2').innerHTML = checkMark;
                resultBadge.innerText = 'Hoàn thành tốt nhiệm vụ';
                resultBadge.className = 'ranking-result-badge rank-good';
            } else if (score >= 50) {
                document.getElementById('rank-check-3').innerHTML = checkMark;
                resultBadge.innerText = 'Hoàn thành nhiệm vụ';
                resultBadge.className = 'ranking-result-badge rank-fair';
            } else {
                document.getElementById('rank-check-4').innerHTML = checkMark;
                resultBadge.innerText = 'Không hoàn thành nhiệm vụ';
                resultBadge.className = 'ranking-result-badge rank-poor';
            }
        }

        function updateKPIScore(subId, itemId, type, val) {
            ...
            if (sub) {
                ...
                if (item) {
                    if (num > item.maxScore) {
                        alert(`Điểm không được vượt quá điểm tối đa (${item.maxScore})`);
                        num = item.maxScore;
                    }
                    if (num < 0) num = 0;
                    item[type] = num;
        ...
        function saveKPIRatingByUser() {
            ...
            lastKpiTimestamp = `${nowStr} (Tự chấm)`;
            saveKPIRatingStorage(lastKpiTimestamp);
            alert(`Đã lưu kết quả tự chấm KPI cho ${kpiTargetUser} (${selectedKpiMonth})!`);
        }
        ...
        function saveKPIRatingByAdmin() {
            ...
            const nowStr = new Date().toLocaleString('vi-VN');
            lastKpiTimestamp = `${nowStr} (Lãnh đạo phê duyệt)`;
            saveKPIRatingStorage(lastKpiTimestamp);
            alert(`Đã lưu và duyệt kết quả đánh giá KPI cho ${kpiTargetUser} (${selectedKpiMonth})!`);
        }

        function editSectionInfo(secKey) {
            const currentTitle = sectionTitles[secKey] || '';
            const currentMax = sectionMaxScores[secKey] || 0;

            const newTitle = prompt(`Nhập tên tiêu đề Mục ${secKey}:`, currentTitle);
            if (newTitle === null) return;

            const newMaxStr = prompt(`Nhập điểm tối đa cho Mục ${secKey}:`, currentMax);
            if (newMaxStr === null) return;

            sectionTitles[secKey] = newTitle.trim();
            sectionMaxScores[secKey] = parseFloatStrict(newMaxStr);

            renderKPITable();
        }

        function addMainSection() {
            const secKey = prompt('Nhập chữ cái đại diện cho Mục lớn mới (ví dụ: D, E...):');
            if (!secKey) return;
            const keyUpper = secKey.trim().toUpperCase();

            if (sectionTitles[keyUpper]) { alert('Mục này đã tồn tại!'); return; }

            const title = prompt(`Nhập tiêu đề cho Mục ${keyUpper}:`, `MỤC ${keyUpper}`);
            if (!title) return;

            const maxScoreStr = prompt(`Nhập tổng điểm tối đa cho Mục ${keyUpper}:`, '10');

            sectionTitles[keyUpper] = title.trim();
            sectionMaxScores[keyUpper] = parseFloatStrict(maxScoreStr);

            addSubsection(keyUpper);
        }

        function addSubsection(secKey) {
            const code = prompt('Nhập mã số La Mã / Nhóm (ví dụ: I, II, III...):', 'I');
            if (!code) return;

            const title = prompt('Nhập tiêu đề nhóm công việc:');
            if (!title) return;

            const maxScoreStr = prompt('Nhập điểm tối đa nhóm:', '10');
            ...
            kpiDataList.push({
                ...
                code: code.trim(),
                title: title.trim(),
                maxScore: parseFloatStrict(maxScoreStr),
                items: []
            });
        ...
        function editSubsection(subId) {
            ...
            const newCode = prompt('Sửa mã nhóm:', sub.code);
            if (newCode !== null) sub.code = newCode.trim();

            const newTitle = prompt('Sửa tiêu đề nhóm:', sub.title);
            if (newTitle !== null) sub.title = newTitle.trim();

            const newMax = prompt('Sửa điểm tối đa nhóm:', sub.maxScore);
            if (newMax !== null) sub.maxScore = parseFloatStrict(newMax);

            renderKPITable();
        }

        function deleteSubsection(subId) {
            if (confirm('Bạn có chắc chắn muốn xóa nhóm này và tất cả các tiêu chí bên trong?')) {
                kpiDataList = kpiDataList.filter(s => s.id !== subId);
                renderKPITable();
            }
        }

        function addItemToSub(subId) {
            const sub = kpiDataList.find(s => s.id === subId);
            if (!sub) return;

            const title = prompt('Nhập nội dung tiêu chí đánh giá chi tiết:');
            if (!title) return;

            const criteria = prompt('Nhập mô tả / yêu cầu đạt được (Tiêu chí):', '');
        ...
        function editKPIItem(subId, itemId) {
            ...
            const newTitle = prompt('Sửa nội dung tiêu chí:', item.title);
            if (newTitle !== null) item.title = newTitle.trim();

            const newCriteria = prompt('Sửa mô tả tiêu chí:', item.criteria);
        ...
        /* --- HÀM XUẤT FILE WORD CHUẨN ĐỊNH DẠNG THEO YÊU CẦU --- */
        function exportKPIWord() {
            if (typeof docx === 'undefined') {
                alert('Thư viện tạo file Word chưa tải xong. Vui lòng thử lại sau giây lát!');
                return;
            }
            ...
            let roleTitle = 'Cán bộ / Nhân viên';
            if (userKpiType === 'leader') roleTitle = 'Lãnh đạo';
            if (userKpiType === 'cleaner') roleTitle = 'Lao Công';
                ...
                new TableRow({
                    ...
                    children: [
                        new TableCell({
                            ...
                            children: [new Paragraph({ children: [new TextRun({ text: "Nội Dung Đánh Giá", bold: true, size: 22 })], alignment: AlignmentType.CENTER })]
                        }),
                        new TableCell({
                            width: { size: 1200, type: WidthType.DXA },
                            verticalAlign: VerticalAlign.CENTER,
                            children: [new Paragraph({ children: [new TextRun({ text: "Điểm tối đa", bold: true, size: 22 })], alignment: AlignmentType.CENTER })]
                        }),
                        new TableCell({
                            width: { size: 1200, type: WidthType.DXA },
                            verticalAlign: VerticalAlign.CENTER,
                            children: [new Paragraph({ children: [new TextRun({ text: "Điểm tự chấm", bold: true, size: 22 })], alignment: AlignmentType.CENTER })]
                        }),
                        new TableCell({
                            width: { size: 1300, type: WidthType.DXA },
                            verticalAlign: VerticalAlign.CENTER,
                            children: [new Paragraph({ children: [new TextRun({ text: "Điểm đánh giá", bold: true, size: 22 })], alignment: AlignmentType.CENTER })]
                        }),
                    ]
                })
            );

            // Dữ liệu Bảng KPI từ web
            const sectionsMap = {};
            kpiDataList.forEach(sub => {
                const sec = sub.section || 'A';
                if (!sectionsMap[sec]) sectionsMap[sec] = [];
            ...
            Object.keys(sectionsMap).sort().forEach(secKey => {
                    ...
                    new TableRow({
                        ...
                        children: [
                            new TableCell({
                                width: { size: 4000, type: WidthType.DXA },
                                verticalAlign: VerticalAlign.CENTER,
                                children: [new Paragraph({ children: [new TextRun({ text: `MỤC ${secKey}: ${secTitleText.toUpperCase()}`, bold: true, size: 22 })] })]
                            }),
                            new TableCell({
                                width: { size: 1200, type: WidthType.DXA },
                                verticalAlign: VerticalAlign.CENTER,
                                children: [new Paragraph({ children: [new TextRun({ text: `${maxSecScore}`, bold: true, size: 22 })], alignment: AlignmentType.CENTER })]
                            }),
                            new TableCell({ width: { size: 1200, type: WidthType.DXA }, children: [new Paragraph({ text: "" })] }),
                            new TableCell({ width: { size: 1300, type: WidthType.DXA }, children: [new Paragraph({ text: "" })] }),
                        ]
                    })
                );
                ...
                sectionsMap[secKey].forEach(sub => {
                        ...
                        new TableRow({
                            ...
                            children: [
                                new TableCell({
                                    width: { size: 4000, type: WidthType.DXA },
                                    verticalAlign: VerticalAlign.CENTER,
                                    children: [new Paragraph({ children: [new TextRun({ text: `${sub.code || globalSubCount}. ${sub.title}`, bold: true, size: 22 })] })]
                                }),
                                new TableCell({
                                    width: { size: 1200, type: WidthType.DXA },
                                    verticalAlign: VerticalAlign.CENTER,
                                    children: [new Paragraph({ children: [new TextRun({ text: `${sub.maxScore}`, bold: true, size: 22 })], alignment: AlignmentType.CENTER })]
                                }),
                                new TableCell({ width: { size: 1200, type: WidthType.DXA }, children: [new Paragraph({ text: "" })] }),
                                new TableCell({ width: { size: 1300, type: WidthType.DXA }, children: [new Paragraph({ text: "" })] }),
                            ]
                    ...
                    if (sub.items) {
                        sub.items.forEach(item => {
                            const m = parseFloatStrict(item.maxScore);
                            const s = parseFloatStrict(item.selfScore);
                            const a = parseFloatStrict(item.adminScore);
                                ...
                                new TableRow({
                                    ...
                                    children: [
                                        new TableCell({
                                            width: { size: 4000, type: WidthType.DXA },
                                            verticalAlign: VerticalAlign.CENTER,
                                            children: [new Paragraph({ children: [new TextRun({ text: `${item.title}`, size: 22 })] })]
                                        }),
                                        new TableCell({
                                            width: { size: 1200, type: WidthType.DXA },
                                            verticalAlign: VerticalAlign.CENTER,
                                            children: [new Paragraph({ children: [new TextRun({ text: `${m.toFixed(1).replace('.0', '')}`, size: 22 })], alignment: AlignmentType.CENTER })]
                                        }),
                                        new TableCell({
                                            width: { size: 1200, type: WidthType.DXA },
                                            verticalAlign: VerticalAlign.CENTER,
                                            children: [new Paragraph({ children: [new TextRun({ text: `${s.toFixed(1).replace('.0', '')}`, size: 22 })], alignment: AlignmentType.CENTER })]
                                        }),
                                        new TableCell({
                                            width: { size: 1300, type: WidthType.DXA },
                                            verticalAlign: VerticalAlign.CENTER,
                                            children: [new Paragraph({ children: [new TextRun({ text: `${a.toFixed(1).replace('.0', '')}`, size: 22 })], alignment: AlignmentType.CENTER })]
                                        }),
                                    ]
                                })
                            );
                ...
                new TableRow({
                    ...
                    children: [
                        new TableCell({
                            width: { size: 4000, type: WidthType.DXA },
                            verticalAlign: VerticalAlign.CENTER,
                            children: [new Paragraph({ children: [new TextRun({ text: "Tổng điểm đánh giá", bold: true, size: 22 })], alignment: AlignmentType.CENTER })]
                        }),
                        new TableCell({
                            width: { size: 1200, type: WidthType.DXA },
                            verticalAlign: VerticalAlign.CENTER,
                            children: [new Paragraph({ children: [new TextRun({ text: `${totalMax.toFixed(1).replace('.0', '')}`, bold: true, size: 22 })], alignment: AlignmentType.CENTER })]
                        }),
                        new TableCell({
                            width: { size: 1200, type: WidthType.DXA },
                            verticalAlign: VerticalAlign.CENTER,
                            children: [new Paragraph({ children: [new TextRun({ text: `${totalSelf.toFixed(1).replace('.0', '')}`, bold: true, size: 22 })], alignment: AlignmentType.CENTER })]
                        }),
                        new TableCell({
                            width: { size: 1300, type: WidthType.DXA },
                            verticalAlign: VerticalAlign.CENTER,
                            children: [new Paragraph({ children: [new TextRun({ text: `${totalAdmin.toFixed(1).replace('.0', '')}`, bold: true, size: 22 })], alignment: AlignmentType.CENTER })]
                        }),
                    ]
                })
            );

            // Bảng Xếp Loại lấy dữ liệu từ web
            const rankCheck1 = totalAdmin >= 90 ? "x" : "";
            const rankCheck2 = (totalAdmin >= 75 && totalAdmin < 90) ? "x" : "";
            const rankCheck3 = (totalAdmin >= 50 && totalAdmin < 75) ? "x" : "";
            const rankCheck4 = totalAdmin < 50 ? "x" : "";

            let resultText = "Chưa xếp loại";
            if (totalAdmin >= 90) resultText = "Hoàn thành xuất sắc nhiệm vụ";
            else if (totalAdmin >= 75) resultText = "Hoàn thành tốt nhiệm vụ";
            else if (totalAdmin >= 50) resultText = "Hoàn thành nhiệm vụ";
            else resultText = "Không hoàn thành nhiệm vụ";
                ...
                new TableRow({
                    ...
                    children: [
                        new TableCell({
                            ...
                            verticalAlign: VerticalAlign.CENTER,
                            children: [new Paragraph({ children: [new TextRun({ text: "STT", bold: true, size: 22 })], alignment: AlignmentType.CENTER })]
                        }),
                        new TableCell({
                            width: { size: 3200, type: WidthType.DXA },
                            verticalAlign: VerticalAlign.CENTER,
                            children: [new Paragraph({ children: [new TextRun({ text: "Mức xếp loại", bold: true, size: 22 })], alignment: AlignmentType.CENTER })]
                        }),
                        new TableCell({
                            width: { size: 2400, type: WidthType.DXA },
                            verticalAlign: VerticalAlign.CENTER,
                            children: [new Paragraph({ children: [new TextRun({ text: "Mức điểm", bold: true, size: 22 })], alignment: AlignmentType.CENTER })]
                        }),
                        new TableCell({
                            width: { size: 1300, type: WidthType.DXA },
                            verticalAlign: VerticalAlign.CENTER,
                            children: [new Paragraph({ children: [new TextRun({ text: "Đạt được", bold: true, size: 22 })], alignment: AlignmentType.CENTER })]
                        }),
                    ]
                }),
                new TableRow({
                    cantSplit: true,
                    children: [
                        new TableCell({ width: { size: 800, type: WidthType.DXA }, verticalAlign: VerticalAlign.CENTER, children: [new Paragraph({ children: [new TextRun({ text: "1", size: 22 })], alignment: AlignmentType.CENTER })] }),
                        new TableCell({ width: { size: 3200, type: WidthType.DXA }, verticalAlign: VerticalAlign.CENTER, children: [new Paragraph({ children: [new TextRun({ text: "Hoàn thành xuất sắc nhiệm vụ", bold: true, size: 22 })] })] }),
                        new TableCell({ width: { size: 2400, type: WidthType.DXA }, verticalAlign: VerticalAlign.CENTER, children: [new Paragraph({ children: [new TextRun({ text: "Từ 90 điểm trở lên", size: 22 })] })] }),
                        new TableCell({ width: { size: 1300, type: WidthType.DXA }, verticalAlign: VerticalAlign.CENTER, children: [new Paragraph({ children: [new TextRun({ text: rankCheck1, bold: true, size: 22 })], alignment: AlignmentType.CENTER })] }),
                    ]
                }),
                new TableRow({
                    cantSplit: true,
                    children: [
                        new TableCell({ width: { size: 800, type: WidthType.DXA }, verticalAlign: VerticalAlign.CENTER, children: [new Paragraph({ children: [new TextRun({ text: "2", size: 22 })], alignment: AlignmentType.CENTER })] }),
                        new TableCell({ width: { size: 3200, type: WidthType.DXA }, verticalAlign: VerticalAlign.CENTER, children: [new Paragraph({ children: [new TextRun({ text: "Hoàn thành tốt nhiệm vụ", bold: true, size: 22 })] })] }),
                        new TableCell({ width: { size: 2400, type: WidthType.DXA }, verticalAlign: VerticalAlign.CENTER, children: [new Paragraph({ children: [new TextRun({ text: "Từ 75 đến dưới 90 điểm", size: 22 })] })] }),
                        new TableCell({ width: { size: 1300, type: WidthType.DXA }, verticalAlign: VerticalAlign.CENTER, children: [new Paragraph({ children: [new TextRun({ text: rankCheck2, bold: true, size: 22 })], alignment: AlignmentType.CENTER })] }),
                    ]
                }),
                new TableRow({
                    cantSplit: true,
                    children: [
                        new TableCell({ width: { size: 800, type: WidthType.DXA }, verticalAlign: VerticalAlign.CENTER, children: [new Paragraph({ children: [new TextRun({ text: "3", size: 22 })], alignment: AlignmentType.CENTER })] }),
                        new TableCell({ width: { size: 3200, type: WidthType.DXA }, verticalAlign: VerticalAlign.CENTER, children: [new Paragraph({ children: [new TextRun({ text: "Hoàn thành nhiệm vụ", bold: true, size: 22 })] })] }),
                        new TableCell({ width: { size: 2400, type: WidthType.DXA }, verticalAlign: VerticalAlign.CENTER, children: [new Paragraph({ children: [new TextRun({ text: "Từ 50 đến dưới 75 điểm", size: 22 })] })] }),
                        new TableCell({ width: { size: 1300, type: WidthType.DXA }, verticalAlign: VerticalAlign.CENTER, children: [new Paragraph({ children: [new TextRun({ text: rankCheck3, bold: true, size: 22 })], alignment: AlignmentType.CENTER })] }),
                    ]
                }),
                new TableRow({
                    cantSplit: true,
                    children: [
                        new TableCell({ width: { size: 800, type: WidthType.DXA }, verticalAlign: VerticalAlign.CENTER, children: [new Paragraph({ children: [new TextRun({ text: "4", size: 22 })], alignment: AlignmentType.CENTER })] }),
                        new TableCell({ width: { size: 3200, type: WidthType.DXA }, verticalAlign: VerticalAlign.CENTER, children: [new Paragraph({ children: [new TextRun({ text: "Không hoàn thành nhiệm vụ", bold: true, size: 22 })] })] }),
                        new TableCell({ width: { size: 2400, type: WidthType.DXA }, verticalAlign: VerticalAlign.CENTER, children: [new Paragraph({ children: [new TextRun({ text: "Dưới 50 điểm", size: 22 })] })] }),
                        new TableCell({ width: { size: 1300, type: WidthType.DXA }, verticalAlign: VerticalAlign.CENTER, children: [new Paragraph({ children: [new TextRun({ text: rankCheck4, bold: true, size: 22 })], alignment: AlignmentType.CENTER })] }),
                    ]
                }),
            ];

            // Trích xuất tháng từ kỳ đánh giá
            let monthVal = "";
            if (selectedKpiMonth && selectedKpiMonth.includes('-')) {
                monthVal = selectedKpiMonth.split('-')[1];
            } else {
                monthVal = String(new Date().getMonth() + 1).padStart(2, '0');
            }

            // Bảng Header chứa Đơn vị (Trái) và Quốc hiệu (Phải) nằm cùng hàng
                            ...
                            new TableCell({
                                ...
                                children: [
                                    new Paragraph({
                                        ...
                                        children: [new TextRun({ text: "TRUNG TÂM KSBT BẮC NINH", bold: true, size: 22 })]
                                    }),
                                    new Paragraph({
                                        alignment: AlignmentType.CENTER,
                                        children: [new TextRun({ text: "KHOA HÓA LÝ", bold: true, underline: {}, size: 22 })]
                                    })
                                ]
                            }),
                            new TableCell({
                                ...
                                children: [
                                    new Paragraph({
                                        alignment: AlignmentType.CENTER,
                                        children: [new TextRun({ text: "CỘNG HÒA XÃ HỘI CHỦ NGHĨA VIỆT NAM", bold: true, size: 22 })]
                                    }),
                                    new Paragraph({
                                        alignment: AlignmentType.CENTER,
                                        children: [new TextRun({ text: "Độc lập - Tự do - Hạnh phúc", bold: true, underline: {}, size: 22 })]
                                    })
                                ]
                            })
                        ]
            ...
            const doc = new Document({
                sections: [{
                    properties: {
                        page: {
                            margin: { top: 1134, bottom: 1134, left: 1417, right: 1134 } // Top/Bottom 2cm, Left 2.5cm, Right 2cm
                        }
                    },
                    children: [
                        headerTable,
                        ...
                        new Paragraph({
                            children: [
                                ...
                                new TextRun({ text: `${kpiTargetUser}`, size: 24 })
                            ],
                            spacing: { after: 100 }
                        }),
                        new Paragraph({
                            children: [
                                new TextRun({ text: "Chức vụ: ", size: 24 }),
                                new TextRun({ text: `${roleTitle}`, size: 24 })
                            ],
                            spacing: { after: 200 }
                        }),
                        new Paragraph({
                            alignment: AlignmentType.CENTER,
                            children: [new TextRun({ text: "PHIẾU THEO DÕI ĐÁNH GIÁ VIÊN CHỨC", bold: true, size: 28 })],
                            spacing: { after: 100 }
                        }),
                        new Paragraph({
                            alignment: AlignmentType.CENTER,
                            children: [new TextRun({ text: `(Kỳ đánh giá tháng ${monthVal} năm 2026)`, italics: true, size: 24 })],
                            spacing: { after: 300 }
                        }),
                        new Table({
                            width: { size: 100, type: WidthType.PERCENTAGE },
                        ...
                        new Paragraph({
                            alignment: AlignmentType.CENTER,
                            children: [new TextRun({ text: "BẢNG XẾP LOẠI", bold: true, size: 24 })],
                            spacing: { after: 100 }
                        }),
                        new Table({
                            width: { size: 100, type: WidthType.PERCENTAGE },
                        ...
                        new Paragraph({
                            children: [
                                new TextRun({ text: "Kết quả xếp loại: ", bold: true, size: 24 }),
                                new TextRun({ text: `${resultText}`, bold: true, size: 24 })
                            ],
                            spacing: { before: 200, after: 300 }
                        }),
                        new Paragraph({
                            alignment: AlignmentType.RIGHT,
                            children: [new TextRun({ text: "Bắc Ninh, ngày....tháng....năm 2026", italics: true, size: 24 })],
                            spacing: { after: 100 }
                        }),
                        new Paragraph({
                            alignment: AlignmentType.RIGHT,
                            children: [new TextRun({ text: "XÁC NHẬN CỦA LÃNH ĐẠO KHOA, TRƯỞNG KHOA                  ", bold: true, size: 24 })],
                            spacing: { after: 100 }
                        })
                    ]
                }]
            ...
            Packer.toBlob(doc).then(blob => {
                saveAs(blob, `Phieu_Danh_Gia_KPI_${kpiTargetUser}_Thang_${monthVal}_2026.docx`);
            }).catch(err => {
